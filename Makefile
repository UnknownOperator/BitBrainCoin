...
pkgdatadir = $(datadir)/bitbraincoincore
pkgincludedir = $(includedir)/bitbraincoincore
pkglibdir = $(libdir)/bitbraincoincore
pkglibexecdir = $(libexecdir)/bitbraincoincore
...
CONFIG_HEADER = $(top_builddir)/src/config/bitbraincoin-config.h
CONFIG_CLEAN_FILES = libbitbraincoinconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libbitbraincoinconsensus.pc.in \
	...
	$(top_srcdir)/src/config/bitbraincoin-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/BitBrainCoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/BitBrainCoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/BitBrainCoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/BitBrainCoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = bitbraincoin-cli
BITCOIN_DAEMON_NAME = bitbraincoind
BITCOIN_GUI_NAME = bitbraincoin-qt
BITCOIN_TX_NAME = bitbraincoin-tx
...
COPYRIGHT_HOLDERS_FINAL = The BitBrainCoin Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = BitBrainCoin Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/BitBrainCoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = bitbraincoincore
PACKAGE_BUGREPORT = https://github.com/bitbraincoinpay/bitbraincoin/issues
PACKAGE_NAME = BitBrainCoin Core
PACKAGE_STRING = BitBrainCoin Core 1.0.0
PACKAGE_TARNAME = bitbraincoincore
PACKAGE_URL = https://bitbraincoin.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/BitBrainCoin
abs_srcdir = /tmp/BitBrainCoin
abs_top_builddir = /tmp/BitBrainCoin
abs_top_srcdir = /tmp/BitBrainCoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/BitBrainCoin/build-aux/install-sh
...
pkgconfig_DATA = libbitbraincoinconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/bitbraincoin-cli.bash-completion \
	       $(top_srcdir)/contrib/bitbraincoin-tx.bash-completion \
	       $(top_srcdir)/contrib/bitbraincoind.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_bitbraincoin_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_bitbraincoin_coverage.info test_bitbraincoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/BitBrainCoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/bitbraincoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/bitbraincoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/bitbraincoin-config.h
$(top_srcdir)/src/config/bitbraincoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/bitbraincoin-config.h src/config/stamp-h1
libbitbraincoinconsensus.pc: $(top_builddir)/config.status $(srcdir)/libbitbraincoinconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/BitBrainCoin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BitBrainCoin-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BitBrainCoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_bitbraincoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_bitbraincoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_bitbraincoin_filtered.info: test_bitbraincoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_bitbraincoin_filtered.info
...
#test_bitbraincoin_coverage.info: baseline_filtered_combined.info test_bitbraincoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitbraincoin_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_bitbraincoin_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitbraincoin_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_bitbraincoin.coverage/.dirstamp:  test_bitbraincoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_bitbraincoin.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_bitbraincoin.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
