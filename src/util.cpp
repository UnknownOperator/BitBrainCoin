...
const char * const BITCOIN_CONF_FILENAME = "bitbraincoin.conf";
const char * const BITCOIN_PID_FILENAME = "bitbraincoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bitbraincoin" is a composite category enabling all BitBrainCoin-related debug output
                if(ptrCategory->count(std::string("bitbraincoin"))) {
...
#else
    const char* pszModule = "bitbraincoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitBrainCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitBrainCoin
    // Mac: ~/Library/Application Support/BitBrainCoin
    // Unix: ~/.bitbraincoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitBrainCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitBrainCoin";
#else
    // Unix
    return pathRet / ".bitbraincoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitbraincoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bitbraincoin.conf
