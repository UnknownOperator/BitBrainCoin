...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in bitbraincoin.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("bitbraincoin.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "bitbraincoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "bitbraincoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "bitbraincoin",               "gobject",                &gobject,                true,  {} },
    { "bitbraincoin",               "voteraw",                &voteraw,                true,  {} },
